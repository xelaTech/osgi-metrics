...
public class ExponentiallyDecayingReservoir implements Reservoir {
	...
	/**
	 * Creates a new {@link ExponentiallyDecayingReservoir} of 1028 elements, which
	 ...
	 *            the reservoir will be towards newer values
	 */
	...
	public ExponentiallyDecayingReservoir(final int size, final double alpha, final Clock clock) {
		...
		nextScaleTime = new AtomicLong(clock.getTick() + RESCALE_THRESHOLD);
	}
	 ...
	 * counts and other quantities based on g(ti − L), and only scale by g(t − L) at
	 * query time. But while g(ti −L)/g(t−L) is guaranteed to lie between zero and
	 * one, the intermediate values of g(ti − L) could become very large. For
	 ...
	 * landmark L′ (and then use this new L′ at query time). This can be done with a
	 * linear pass over whatever data structure is being used."
	 */
			...
			if (nextScaleTime.compareAndSet(next, now + RESCALE_THRESHOLD)) {
				...
				if (Double.compare(scalingFactor, 0) == 0) {
					...
					for (final Double key : keys) {
						...
						final WeightedSample newSample = new WeightedSample(sample.value,
